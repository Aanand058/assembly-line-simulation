...
namespace sdds {
    ...
    Station::Station(const std::string& line) : m_id(++id_generator)
    {
        ...
        this->m_name = utilities.extractToken(line, next_p, full);

        this->m_nextSerialNo = stoi(utilities.extractToken(line, next_p, full));

        this->m_noOfItems = stoi(utilities.extractToken(line, next_p, full));

        this->m_widthField = max(utilities.getFieldWidth(), m_widthField);

        this->m_desc = utilities.extractToken(line, next_p, full);
    }
